---
layout: post
title:  PHP文件上传类(支持单文件上传，也支持多文件上传) 
category: PHP
tag: [php]
date : 2015-10-03
excerpt: PHP文件上传类(支持单文件上传，也支持多文件上传) 
---
...
**文件上传类的应用过程：**
  ...
  本列的文件上传类FileUpload，不仅支持单文件上传，也支持多文件上传。
  在处理方式上没有区别，只不过在编写上传表单时，多个文件上传一定要以数组方式传递给服务器。
  单个文件上传表单如下所示：

![fileClass_1](/res/img/blog/PHP学习/fileClass_1.jpg)

  上面表单都是，都将提交的位置指向了同一个文件upload.php，
